...
def clone_repo(repo_url: str = REPO_URL, directory: str = None) -> str:
    ...
    try:
        ...
        target_dir = directory or repo_url.split('/')[-1].replace('.git', '')
        if os.path.exists(target_dir):
            return f"Error: Directory '{target_dir}' already exists!"
        ...
        if os.path.exists(target_dir):
            ...
            subprocess.run(["cd", target_dir], capture_output=True, text=True, check=True)
            
            return (f"Repository successfully cloned!\n"
                   f"Location: {os.path.abspath(target_dir)}\n"
                   f"Output:\n{result.stdout}")
        else:
            return "Error: Repository appears to be cloned but directory not found!"
...
system_prompt = (
    ...
    f" Start with cloning the repository, generate the code"
    f" for the requested feature or bug fix, test the code, review the code"
    f" with Coder Agent. Create a pull request for output. The QA Tester will test"
...
code_agent = create_agent(
    ...
    You start with cloning the repository. You leverage the existing codebase and 
    Python REPL to find files, write and test code. You generate unit tests that 
    covers the new code completely. You create a new branch, commit the code and 
    create a  pull request in the cloned repository only. You follow the existing 
